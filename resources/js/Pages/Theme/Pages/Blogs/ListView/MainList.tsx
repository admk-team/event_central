                                    ...
                                    <div className="row g-4">
                                        ...
                                        <div className="col-xxl-9 col-lg-7">
                                            ...
                                            <div className="d-flex align-items-center gap-2 mb-3 flex-wrap">
                                                <span className="text-muted"><i className="ri-calendar-event-line me-1"></i> {item.date}</span> | <span className="text-muted"><i className="ri-eye-line me-1"></i> {item.views}</span> | <Link href="/pages-profile"><i className="ri-user-3-line me-1"></i> Admin</Link>
                                            </div>
                                            <p className="text-muted mb-2">{item.description}</p>
                                            <Link href="/pages-blog-overview" className="text-decoration-underline">Read more <i className="ri-arrow-right-line"></i></Link>
                                            <div className="d-flex align-items-center gap-2 mt-3 flex-wrap">
                                                {item.tags.map((item, idx) => (
                                                    <Link href="#!" key={idx} className="badge text-success bg-success-subtle">{item}</Link>
                                                ))}
                                            </div>
                                        </div>
                                    </div>
