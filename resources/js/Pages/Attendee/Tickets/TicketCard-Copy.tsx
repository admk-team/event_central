...
const TicketCard = ({ ticket, onAddToCart, onRemoveFromCart }: any) => {
    ...
    const [price, setPrice] = useState(parseFloat(ticket.base_price));
    ...
    const validateCode = () => {
        ...
        axios
            .post(
                route("attendee.validateCode.post", [ticket.id, discountCode])
            )
            .then((response) => {
    ...
    const createQtyOptions = (items: any) => {
        const listItems = []
        for (let i = 0; i < items; i++) {
            listItems.push(<option value={i}>{i}</option>)
        }
        return listItems
    }
                ...
                <Card.Body className="p-4 m-2" style={{ minHeight: "400px" }}>
                    ...
                    <div>
                        ...
                        {/* <ul
                            ...
                            id="AttendeeTicketFeature"
                        >
                            {!viewAllFeatures &&
                                ticket.all_features.length > 0 &&
                                ticket.all_features.map(
                                    (feature: any) =>
                                        feature.selected > 0 && (
                                            <li key={feature.id}>
                                                <div className="d-flex">
                            ...
                            {viewAllFeatures &&
                                ticket.all_features.length > 0 &&
                                ticket.all_features.map((feature: any) => (
                                    <li key={feature.id}>
                                        <div className="d-flex">
                                            {feature.selected > 0 ? (
                                                <div className="flex-shrink-0 text-success me-1">
