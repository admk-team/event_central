...
const Index = ({ baseTemplate }: any) => {
    ...
    const previewTemplate = (id: number) => {
        const url = route('organizer.events.email-template.show', id);
    ...
    const editTemplate = (id: number) => {
        router.visit(route('organizer.events.email-template.edit', id));
    };

    const deleteTemplate = (id: number) => {
        if (confirm(`Are you sure you want to delete template #${id}? This action cannot be undone.`)) {
            router.delete(route('organizer.events.base.template.delete', id));
        }
    };
            ...
            <div className='row'>
                <div className='col-sm-4'>
                    <button
                        ...
                        onClick={() => router.visit(route('organizer.events.email-template.create'))}
                    >
                        Create New Template
                    </button>
                </div>
