...
const QuestionList: React.FC<Props> = ({
  ...
  const appendNewAnswer = (newAnswer: any, questionId: number) => {
    setQuestions((prevQuestions) =>
      prevQuestions.map((q) => (q.id === questionId ? { ...q, answers: [...q.answers, newAnswer] } : q))
    );
  };
    ...
    if (selectedQuestionId) {
      router.put(
        ...
        { content: editContent },
        {
          ...
          onSuccess: () => {
            setQuestions((prev) =>
              prev.map((q) => (q.id === selectedQuestionId ? { ...q, content: editContent } : q))
            );
        ...
        { content: editContent },
        {
          ...
          onSuccess: () => {
            setQuestions((prev) =>
              prev.map((q) => ({
                ...q,
                answers: q.answers.map((a) => (a.id === selectedAnswerId ? { ...a, content: editContent } : a)),
              }))
            );
    ...
    <div className="space-y-4">
      {sortedQuestions.length === 0 ? (
        <Card className="text-center p-4">
          <p className="text-muted mb-0">
            {t("No questions have been submitted yet. Stay tuned for attendee engagement!")}
          </p>
        </Card>
      ) : (
          ...
          <Card key={question.id} className="shadow-sm mb-4" id={`question-${question.id}`}>
            <Card.Body>
                ...
                <Col>
                  ...
                  <p className="text-muted fs-12 mb-0">
                    ...
                    <Badge bg="light" text="dark">
                      {question.user?.first_name || t("Anonymous")}
                    </Badge>{" "}
              ...
              <Row className="mt-2 align-items-center">
                ...
                {user?.role === "organizer" && (
                  <Col xs="auto" className="ms-auto">
                    <Button variant="success" size="sm" onClick={() => handleOpenAnswerModal(question.id)}>
                      <i className="ri-reply-line me-1"></i> {t("Answer")}
                    </Button>
                  </Col>
                )}
              </Row>
      ...
      <Modal show={showAnswerModal} onHide={handleCloseAnswerModal} centered>
        <Modal.Header closeButton>
          <Modal.Title>{t("Answer Question")}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {selectedQuestionId && (
      ...
      <Modal show={showEditQuestionModal} onHide={() => setShowEditQuestionModal(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>{t("Edit Question")}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form>
            <Form.Group>
              <Form.Label>{t("Question Content")}</Form.Label>
              <Form.Control
                as="textarea"
                rows={3}
                value={editContent}
      ...
      <Modal show={showEditAnswerModal} onHide={() => setShowEditAnswerModal(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>{t("Edit Answer")}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form>
            <Form.Group>
              <Form.Label>{t("Answer Content")}</Form.Label>
              <Form.Control
                as="textarea"
                rows={3}
                value={editContent}
