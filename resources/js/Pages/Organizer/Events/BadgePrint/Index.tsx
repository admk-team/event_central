...
function Index({ attendees, eventApp }: { attendees: any; eventApp: any }) {
    ...
    const printSingleBadge = (index: string) => {
        const printContent = document.getElementById(index);
        if (printContent) {
            // Hide everything else except the specific badge
            const allContent = document.querySelector(".printable");
            if (allContent) {
                allContent.style.visibility = "hidden";
            }

            // Make the specific badge visible
    ...
    return (
        <React.Fragment>
            ...
            <div className="printable-single">
                {filteredAttendees.map((attendee: any, index: number) =>
                    attendee.qr_codes.map((qr: string, idx: number) => (
                        <div key={idx} className="passWrapper print-page-break">
                            <div className="passes-container">
