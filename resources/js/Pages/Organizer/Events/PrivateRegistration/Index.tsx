...
function Index({ data }: any) {
  ...
  return (
    <>
      <Head title={t("Private Registration")} />
      <div className="page-content">
        <Container fluid>
          <BreadCrumb2
            title={t("Private Registration")}
            items={[
              {
                title: t("Private Registration via Email"),
          ...
          <Row className="mt-4">
            <Col md={9} lg={9}>
              <FormGroup className="mb-3">
                <FormLabel>{t("Send invites via Email")}</FormLabel>
                <Select
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter" && inputValue.trim()) {
                      ...
                      if (/^\S+@\S+\.\S+$/.test(email)) {
                        const exists = selectEmails.some(
          ...
          <Row className="mt-4">
            <Col xs={12}>
              <DataTable
                data={data}
                columns={columns}
                title={t("Private Registration Emails")}
                actions={[
                  {
                    render: (dataTable: any) => (
                      <HasPermission permission="delete_private_registration">
                        <Button
                          ...
                          onClick={() =>
                            deleteManyAction(
                              dataTable.getSelectedRows().map((row: any) => row.id)
                            )
                          }
                        >
                          <i className="ri-delete-bin-5-line"></i>{" "}
