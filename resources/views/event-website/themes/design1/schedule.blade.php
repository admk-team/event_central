...
@section('style')
    ...
    @vite(['resources/css/design1/schedule_styles.css'])
@endsection
...
<section id="schedule"
  class="schedule register register--grid mt-2"
  x-data="{
    selectedTracks: [],
    selectedPlatforms: [],
      ...
      <div class="tracks">
        ...
        <div class="tracks-filter">
          ...
          <template x-if="selectedTracks.length">
            <button @click="clearTrackSelections" class="btn btn-secondary" aria-label="Clear track filters">&#10006;</button>
          </template>
        </div>
      </div>
    @endif
    ...
    <div class="locations">
      ...
      <div class="locations-filter">
        ...
        <template x-if="selectedPlatforms.length">
          <button @click="clearPlatformSelections" class="btn btn-secondary" aria-label="Clear location filters">&#10006;</button>
        </template>
      </div>
    </div>
        ...
        <div class="schedule-day {{ $loop->first ? 'active' : '' }}" id="day{{ $loop->index + 1 }}">
          <div class="schedule-timeline">
            @foreach ($date->eventSessions()->orderBy('start_time')->get() as $session)
              <div
                x-show="hasSelectedTracks({{ $session->tracks->pluck('id') }}) && hasSelectedPlatform({{ $session->event_platform_id }})"
                class="timeline-item"
                type="button"
                  ...
                  <div class="modal-content">
                    <div class="modal-body px-4">
                      ...
                      <div class="text-center">
                        ...
                        @endif
                        <p>
                          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill:#000">
                      ...
                      @isset($session->eventSpeakers)
                        @foreach ($session->eventSpeakers as $speaker)
                          ...
                          <div class="text-center d-flex justify-content-center align-items-center gap-1">
                            <div
                              class="speaker-trigger d-inline-flex align-items-center gap-1"
                          ...
                          <div class="modal fade" id="{{ $speakerModalId }}" tabindex="-1" aria-labelledby="modalLabel{{ $speakerModalId }}" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered">
                              <div class="modal-content pt-3">
                                <div class="d-flex justify-content-center">
                                  <img class="rounded-circle" src="{{ $speaker->avatar ?: $event->logo }}" width="150" height="150" alt="{{ $speaker->name }}">
                                </div>
                                <div class="px-4 text-center">
                                  ...
                                  <div class="bio-container">
                                    @if (!empty($speaker->bio) && strlen($speaker->bio) > 100)
                                      <p class="text-muted bio-text short-bio" id="shortBio{{ $speakerKey }}">{{ substr($speaker->bio, 0, 100) }}...</p>
                                      <p class="text-muted bio-text full-bio d-none" id="fullBio{{ $speakerKey }}">{{ $speaker->bio }}</p>
                                      <span class="btn show-more-btn p-0" data-speaker-key="{{ $speakerKey }}">Show More</span>
                                ...
                                <div class="modal-footer text-center d-flex justify-content-center gap-3">
                                  @if ($speaker->web)      <a target="_blank" href="{{ $speaker->web }}"      aria-label="Website"><i class="bi bi-globe"></i></a>@endif
                                  @if ($speaker->linkedin) <a target="_blank" href="{{ $speaker->linkedin }}" aria-label="LinkedIn"><i class="bi bi-linkedin"></i></a>@endif
                                  @if ($speaker->facebook) <a target="_blank" href="{{ $speaker->facebook }}" aria-label="Facebook"><i class="bi bi-facebook"></i></a>@endif
                                  @if ($speaker->twitter)  <a target="_blank" href="{{ $speaker->twitter }}"  aria-label="Twitter/X"><i class="bi bi-twitter-x"></i></a>@endif
                                  @if ($speaker->instagram)<a target="_blank" href="{{ $speaker->instagram }}"aria-label="Instagram"><i class="bi bi-instagram"></i></a>@endif
                                </div>
                              </div>
                            </div>
                          </div>
...
(function(){
  ...
  document.addEventListener('click', function(e){
    ...
    if(!trigger) return;
    if(transitioning) return; // guard against double taps
    transitioning = true;
    ...
    if(!id){ transitioning = false; return; }

    let targetEl = document.getElementById(id);
    if(!targetEl){ transitioning = false; return; }

    const parentModalEl = trigger.closest('.modal');

    // Hoist once if still nested
    if (parentModalEl && parentModalEl.contains(targetEl) && targetEl.parentElement !== document.body) {
      document.body.appendChild(targetEl);
    }

    // Blur to avoid aria-hidden warning
    if (document.activeElement) document.activeElement.blur();

    const showTarget = () => {
      const next = bootstrap.Modal.getInstance(targetEl) || new bootstrap.Modal(targetEl, { focus: true });
      // If already visible, just ensure focus; else show.
      if (!targetEl.classList.contains('show')) {
        targetEl.addEventListener('shown.bs.modal', () => { transitioning = false; }, { once:true });
        next.show();
      } else {
        transitioning = false;
    ...
    if (parentModalEl) {
      ...
      const current = bootstrap.Modal.getInstance(parentModalEl) || new bootstrap.Modal(parentModalEl);
...
<style>
.speaker-trigger { cursor:pointer; }
.speaker-trigger:hover{ opacity:.95; }
</style>
@endsection
